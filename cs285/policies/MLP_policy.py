...
class MLPPolicy(BasePolicy):
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        self.policy_vars = [v for v in tf.global_variables() if policy_scope in v.name and 'train' not in v.name] ###这里改过了############
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(-1*self.loss)
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            ...
            _, loss = self.sess.run([self.baseline_update_op, self.baseline_loss], feed_dict={self.observations_pl: observations, self.targets_n:targets_n})
        return loss
